...
//		getEvo(response.body().getId());
		...
		pokemon.setPesoPokemon(String.valueOf(peso));

		altura = pokemonapi.getHeight();
		pokemon.setAlturaPokemon(String.valueOf(altura));

		numPokedex = pokemonapi.getNumPokedex();
		pokemon.setNumPokemon(String.valueOf(numPokedex));
...
//		img = new Image(pokemonapi.getSprites().getFrontDefault().toString());
		...
		if (poSpecies.getEvolFrom() != null) {
			preevoluciona = true;
			pokemon.setPreevoPokemon("SI");
		}

		if (poSpecies.getEvolTo() != null) {
			evoluciona = true;
			pokemon.setEvoPokemon("SI");
		}
